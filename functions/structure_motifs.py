...
def plot_filter_logo(filter_outs, filter_size, seqs, out_prefix, raw_t=0, maxpct_t=None):
    ...
    # make weblogo
    if filter_count > 0:
        weblogo_cmd = 'weblogo %s < %s.fa > %s.pdf' % (weblogo_opts, out_prefix, out_prefix)
...
def structure_motif(args):
    ...
    str_path = os.path.join(set_path, f'ACGU')

    pos_test = np.load(os.path.join(set_path, '../pos_inf.npy')).astype(np.float32)
    ss_test = np.load(os.path.join(set_path, '../ss.npy')).astype(np.float32)
    nlp_test_adj = np.load(os.path.join(set_path, '../adj.npy')).astype(np.float32)
    nlp_test_node = np.load(os.path.join(set_path, '../node_embedding_inf.npy')).astype(np.float32)
    seq_test_dimone = con_seq_1D(os.path.join(set_path, f'Sequential_Feature/RNAonly', 'encoding_features/'))
    y_test = np.load(os.path.join(set_path, '../label.npy'))
    ...
    data_dic = [pos_test, ss_test, nlp_test_node, nlp_test_adj, seq_test_dimone]
