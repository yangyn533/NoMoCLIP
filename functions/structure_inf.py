...
def parser_secondary(args):
    ...
    fastapath = os.path.join(out_path, '../combined_profile.txt')
    secondary = read_fasta(fastapath)
    save_path = os.path.join(out_path, '../ss.npy').format('test')
...
def ss2onehot(args):
    ...
    data = np.load(os.path.join(out_path, '../ss.npy'))
    max_indices = np.argmax(data, axis=-1)
    one_hot = np.eye(5)[max_indices]
    np.save(os.path.join(out_path, '../ss_onehot.npy'), one_hot)
